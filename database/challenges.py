...
def create(g,q,a,cat,pts):
    with db() as c:
        c.execute("UPDATE challenges SET status='cancelled' WHERE group_id=? AND status='active'",(g,)); r=c.execute('INSERT INTO challenges(group_id,question,answer,category,points,created) VALUES(?,?,?,?,?,?)',(g,q,a,cat,pts,now())); return r.lastrowid
def win(cid,uid):
    with db() as c:
