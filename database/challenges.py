...
def create(g,q,a,cat,pts,media_url=None,media_title=None):
    with db() as c:
        c.execute("UPDATE challenges SET status='cancelled' WHERE group_id=? AND status='active'",(g,))
        r=c.execute("INSERT INTO challenges(group_id,question,answer,category,points,status,created,media_url,media_title,media_revealed) VALUES(?,?,?,?,?,'active',?,?,?,0)",(g,q,a,cat,pts,now(),media_url,media_title))
        return r.lastrowid
def win(cid,uid):
    with db() as c:
