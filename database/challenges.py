...
def create(g,q,a,cat,pts,media_url=None,media_title=None,options_json=None):
    with db() as c:
        ...
        r=c.execute("INSERT INTO challenges(group_id,question,answer,category,points,status,created,media_url,media_title,media_revealed,options_json) VALUES(?,?,?,?,?,'active',?,?,?,?,?)",(g,q,a,cat,pts,now(),media_url,media_title,0,options_json))
        return r.lastrowid
def win(cid,uid):
    with db() as c:
