...
def migrate_db():
    ...
    with db() as c:
        ...
        # Migrate feature settings used by the daily chest/battle systems.
        if c.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='feature_settings'").fetchone():
            for column, definition in (("chest_duration_minutes", "INTEGER DEFAULT 10"),):
                _add_column(c, "feature_settings", column, definition)
        ...
        if c.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='battles'").fetchone():
            for column, definition in (("question", "TEXT"),("answer", "TEXT"),("options_json", "TEXT"),("answered", "TEXT")):
                _add_column(c, "battles", column, definition)
...
def init_db():
    with db() as c:
        ...
        CREATE TABLE IF NOT EXISTS chest_rounds(id INTEGER PRIMARY KEY AUTOINCREMENT,group_id INTEGER,started TEXT,ends TEXT,active INTEGER DEFAULT 1);
