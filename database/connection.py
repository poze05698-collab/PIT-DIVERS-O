...
def migrate_db():
    """Apply only additive migrations so existing data is preserved.

    Older releases used different names for group settings and the original
    users table had only five columns.  Never recreate/drop these tables.
    """
    with db() as c:
        # USERS: older database = id, username, first_name, first_seen, last_seen
        ...
        # GROUPS: normalize old setting names to the names used by the current code.
        if c.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='groups'").fetchone():
            ...
            # Always copy legacy values when the old columns exist.  Do not
            # condition this on the new column being NULL: SQLite's DEFAULT 1
            # would otherwise hide an old value of 0.
