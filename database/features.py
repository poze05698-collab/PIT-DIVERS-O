...
def init_features():
    with db() as c:
        ...
        CREATE TABLE IF NOT EXISTS feature_settings(group_id INTEGER PRIMARY KEY,
          ...
          howto_enabled INTEGER DEFAULT 1, howto_text TEXT DEFAULT '') ;
...
def set_value(gid, field, value):
    ensure_group(gid)
    allowed={'xp_per_answer','combo_bonus','chest_points','mission_target','battle_points','event_multiplier','howto_text'}
    if field not in allowed: raise ValueError(field)
