...
from database.points import active_week, previous_week
...
def init_features():
    with db() as c:
        ...
        CREATE TABLE IF NOT EXISTS memory_games(id INTEGER PRIMARY KEY AUTOINCREMENT,group_id INTEGER,question TEXT,answer TEXT,status TEXT DEFAULT 'active',winner INTEGER,created TEXT,expires TEXT);
        ...
        memory_cols={r[1] for r in c.execute('PRAGMA table_info(memory_games)').fetchall()}
        if 'expires' not in memory_cols:
            c.execute("ALTER TABLE memory_games ADD COLUMN expires TEXT")

        chest_cols={r[1] for r in c.execute('PRAGMA table_info(chest_releases)').fetchall()}
        if 'message_id' not in chest_cols:
            c.execute("ALTER TABLE chest_releases ADD COLUMN message_id INTEGER")
        if 'cancelled' not in chest_cols:
            c.execute("ALTER TABLE chest_releases ADD COLUMN cancelled INTEGER NOT NULL DEFAULT 0")
        if 'message_deleted' not in chest_cols:
            c.execute("ALTER TABLE chest_releases ADD COLUMN message_deleted INTEGER NOT NULL DEFAULT 0")
...
def set_value(gid, field, value):
    ensure_group(gid)
    allowed={'xp_per_answer','combo_bonus','chest_points','mission_target','mission_reward','battle_points','event_multiplier','chest_duration_minutes','howto_text'}
    if field not in allowed: raise ValueError(field)
...
def award(gid, uid, base_points, category='Geral'):
    ...
    if s['achievements_enabled']:
        ...
        total_week=0
        with db() as c:
            r=c.execute('SELECT points FROM points WHERE group_id=? AND user_id=? AND week=?',(gid,uid,active_week(gid))).fetchone(); total_week=r['points'] if r else 0
...
def chest_daily_count(gid):
    ...
    with db() as c:
        ...
        r=c.execute('SELECT COUNT(*) n FROM chest_releases WHERE group_id=? AND day=? AND cancelled=0',(gid,day)).fetchone()
        return int(r['n'] or 0)
...
def release_chest(gid, minutes=None):
    ...
    with db() as c:
        ...
        count=int(c.execute('SELECT COUNT(*) n FROM chest_releases WHERE group_id=? AND day=? AND cancelled=0',(gid,day)).fetchone()['n'] or 0)
        if count>=3:
            return None, 'limit'
        # Evita dois baús simultaneamente; o admin libera o próximo após o anterior expirar.
        active=c.execute('SELECT id FROM chest_releases WHERE group_id=? AND day=? AND active=1 AND cancelled=0 AND active_until>? LIMIT 1',(gid,day,created)).fetchone()
        if active:
            return None, 'active'
        cur=c.execute('INSERT INTO chest_releases(group_id,day,points,active_until,created,active,cancelled,message_deleted) VALUES(?,?,?,?,?,?,?,?)',(gid,day,pts,until,created,1,0,0))
        rid=cur.lastrowid
...
def chest_release_status(gid):
    ...
    with db() as c:
        c.execute('UPDATE chest_releases SET active=0 WHERE group_id=? AND day=? AND active=1 AND active_until<=?',(gid,day,now()))
        r=c.execute('SELECT COUNT(*) n FROM chest_releases WHERE group_id=? AND day=? AND cancelled=0',(gid,day)).fetchone()
        active=c.execute('SELECT * FROM chest_releases WHERE group_id=? AND day=? AND active=1 AND cancelled=0 ORDER BY id DESC LIMIT 1',(gid,day)).fetchone()
    return int(r['n'] or 0), active

def set_chest_release_message_id(release_id, message_id):
    with db() as c:
        c.execute('UPDATE chest_releases SET message_id=? WHERE id=?',(int(message_id),int(release_id)))

def cancel_chest_release(release_id):
    with db() as c:
        c.execute('UPDATE chest_releases SET active=0,cancelled=1 WHERE id=? AND active=1',(int(release_id),))

def expired_chest_messages():
    """Retorna postagens expiradas e marca a liberação como inativa.
    O histórico permanece no banco e a exclusão da mensagem é idempotente.
    """
    current=now(); out=[]
    with db() as c:
        rows=c.execute('SELECT id,group_id,message_id FROM chest_releases WHERE active_until<=? AND message_id IS NOT NULL AND message_deleted=0 AND cancelled=0',(current,)).fetchall()
        for r in rows:
            c.execute('UPDATE chest_releases SET active=0 WHERE id=? AND active=1',(r['id'],))
            out.append((int(r['id']),int(r['group_id']),int(r['message_id'])))
    return out

def mark_chest_message_deleted(release_id):
    with db() as c:
        c.execute('UPDATE chest_releases SET message_deleted=1 WHERE id=?',(int(release_id),))

def claim_chest_release(release_id, gid, uid):
    """Resgate atômico: um usuário só recebe uma vez e nunca após expirar."""
    current=now()
    with db() as c:
        r=c.execute(
            'SELECT * FROM chest_releases WHERE id=? AND group_id=? AND active=1 AND cancelled=0 AND active_until>?',
            (release_id,gid,current)
        ).fetchone()
        if not r:
            c.execute('UPDATE chest_releases SET active=0 WHERE id=? AND active=1 AND active_until<=?',(release_id,current))
            return None
        pts=max(1,int(r['points']))
        cur=c.execute(
            'INSERT OR IGNORE INTO chest_claims(release_id,group_id,user_id,points,claimed) VALUES(?,?,?,?,?)',
            (release_id,gid,uid,pts,current)
        )
        if cur.rowcount != 1:
            return None
        return pts

def unclaim_chest_release(release_id, gid, uid):
    """Remove um claim recém-criado quando o crédito de pontos falhou."""
    with db() as c:
        c.execute(
            "DELETE FROM chest_claims WHERE release_id=? AND group_id=? AND user_id=?",
            (int(release_id), gid, uid),
        )


def deactivate_expired_chests():
    with db() as c:
        c.execute('UPDATE chest_releases SET active=0 WHERE active=1 AND active_until<=?',(now(),))

def claim_mission(gid,uid):
    """Resgate atômico da missão diária: só uma requisição recebe a recompensa."""
    s=settings(gid); day=_local_day(); pts=max(1,int(s['mission_reward']))
    with db() as c:
        cur=c.execute(
            'UPDATE missions SET claimed=1 WHERE group_id=? AND user_id=? AND day=? '
...
def unclaim_mission(gid, uid):
    """Reabre a missão se a recompensa não pôde ser registrada no saldo."""
    day = _local_day()
    with db() as c:
        c.execute(
            "UPDATE missions SET claimed=0 WHERE group_id=? AND user_id=? AND day=? AND claimed=1",
            (gid, uid, day),
        )
...
def close_season(gid, week=None, prizes=None):
    """Compatibilidade: delega o fechamento manual para database.points."""
    from database.points import close_season as _close
    result = _close(gid, week=week, prizes=prizes)
