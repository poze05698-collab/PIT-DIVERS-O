...
def release_chest(gid, minutes=None):
    ...
    with db() as c:
        ...
        if active:
            return None, 'active'
        cur=c.execute('INSERT INTO chest_releases(group_id,day,points,active_until,created,active) VALUES(?,?,?,?,?,1)',(gid,day,pts,until,created))
        rid=cur.lastrowid
...
def expired_chest_messages():
    """Desativa e retorna (group_id, message_id) dos baús expirados ainda publicados."""
    current=now(); out=[]
    with db() as c:
        ...
        for r in rows:
            ...
            if r['message_id']:
                out.append((int(r['group_id']),int(r['message_id'])))
    return out
