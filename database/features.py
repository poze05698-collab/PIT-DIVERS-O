...
def init_features():
    with db() as c:
        ...
        CREATE TABLE IF NOT EXISTS feature_settings(group_id INTEGER PRIMARY KEY,
          ...
          mission_target INTEGER DEFAULT 5, mission_reward INTEGER DEFAULT 10, battle_points INTEGER DEFAULT 15, event_multiplier INTEGER DEFAULT 2,
        ...
        CREATE TABLE IF NOT EXISTS chests(group_id INTEGER,user_id INTEGER,day TEXT,points INTEGER DEFAULT 0,claimed INTEGER DEFAULT 0,
          PRIMARY KEY(group_id,user_id,day));
        CREATE TABLE IF NOT EXISTS battles(id INTEGER PRIMARY KEY AUTOINCREMENT,group_id INTEGER,challenger INTEGER,opponent INTEGER,status TEXT DEFAULT 'pending',winner INTEGER,created TEXT,points INTEGER DEFAULT 15,question TEXT DEFAULT '',answer TEXT DEFAULT '',options_json TEXT DEFAULT '',message_id INTEGER,expires TEXT DEFAULT '',answered TEXT DEFAULT '');
        ...
        if 'battle_points' not in cols: c.execute("ALTER TABLE feature_settings ADD COLUMN battle_points INTEGER DEFAULT 15")
        if 'event_multiplier' not in cols: c.execute("ALTER TABLE feature_settings ADD COLUMN event_multiplier INTEGER DEFAULT 2")
        # Battle fields used by the persistent 1x1 game.
        bcols={r[1] for r in c.execute('PRAGMA table_info(battles)').fetchall()}
        if 'question' not in bcols: c.execute("ALTER TABLE battles ADD COLUMN question TEXT DEFAULT ''")
        if 'answer' not in bcols: c.execute("ALTER TABLE battles ADD COLUMN answer TEXT DEFAULT ''")
        if 'options_json' not in bcols: c.execute("ALTER TABLE battles ADD COLUMN options_json TEXT DEFAULT ''")
        if 'message_id' not in bcols: c.execute("ALTER TABLE battles ADD COLUMN message_id INTEGER")
        if 'expires' not in bcols: c.execute("ALTER TABLE battles ADD COLUMN expires TEXT DEFAULT ''")
        if 'answered' not in bcols: c.execute("ALTER TABLE battles ADD COLUMN answered TEXT DEFAULT ''")
...
def set_value(gid, field, value):
    ensure_group(gid)
    allowed={'xp_per_answer','combo_bonus','chest_points','mission_target','mission_reward','battle_points','chest_duration_minutes','howto_text'}
    if field not in allowed: raise ValueError(field)
...
def daily_status(gid,uid):
    s=settings(gid); day=datetime.now(timezone.utc).date().isoformat()
    with db() as c:
...
def activate_chest(gid, minutes):
    ensure_group(gid)
    minutes=max(1,min(1440,int(minutes)))
    from datetime import timedelta
    until=(datetime.now(timezone.utc)+timedelta(minutes=minutes)).isoformat()
    with db() as c:
        c.execute('UPDATE feature_settings SET chest_active_until=?,chest_duration_minutes=? WHERE group_id=?',(until,minutes,gid))
    return until

def chest_active(gid):
    s=settings(gid)
    until=str(s['chest_active_until'] or '')
    return bool(s['chest_enabled'] and until and until>now())

def deactivate_chest(gid):
    ensure_group(gid)
    with db() as c: c.execute("UPDATE feature_settings SET chest_active_until='' WHERE group_id=?",(gid,))

def claim_chest(gid,uid):
    s=settings(gid)
    if not s['chest_enabled'] or not chest_active(gid): return None
    day=datetime.now(timezone.utc).date().isoformat()
    pts=max(1,int(s['chest_points']))
    with db() as c:
        cur=c.execute('INSERT OR IGNORE INTO chests(group_id,user_id,day,points,claimed) VALUES(?,?,?,?,1)',(gid,uid,day,pts))
