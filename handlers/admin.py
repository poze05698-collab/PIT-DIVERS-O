...
from database.features import close_season
...
def show_group_text(g):
    return (
        ...
        f"🎁 Prêmios TOP 3:\n"
        f"🥇 <b>{g['prize1'] or g['prize'] or 'não configurado'}</b>\n"
        f"🥈 <b>{g['prize2'] or 'não configurado'}</b>\n"
        f"🥉 <b>{g['prize3'] or 'não configurado'}</b>\n\n"
...
async def prizes(c):
    ...
    if not gs:
        ...
        text += "".join(
            f"• {g['title'] or 'Grupo'}: "
            f"<b>{g['prize'] or 'não configurado'}</b>\n"
            for g in gs
        )
...
async def week_cb(c):
    ...
    from database.points import wk
    week=wk()
    result=close_season(gid,week,prizes)
    if not result:
        return await c.answer("Não há pontuação nesta semana ou ela já foi encerrada.",show_alert=True)
    medals=['🥇','🥈','🥉']; lines=['🏆 <b>SEMANA ENCERRADA!</b>','']
    for i,r in enumerate(result):
        lines.append(f'{medals[i]} <a href="tg://user?id={r["user_id"]}">{r["first_name"] or "Membro"}</a> — <b>{r["points"]} pts</b>\n🎁 {prizes[i]}')
    lines.append('\n🔥 A próxima semana seguirá normalmente no próximo ciclo semanal.')
    try: await c.bot.send_message(gid,'\n'.join(lines))
    except Exception: pass
    await c.answer("Semana fechada e premiação publicada.",show_alert=True)
    await group(c)
