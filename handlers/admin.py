...
async def safe_edit(message, text, reply_markup=None):
    """Edita o painel sem gerar erro quando nada mudou."""
    try:
...
async def interact_send(c):
    ...
    if kind=='visual':
        ...
        category_map={
            ...
            'vf':'V/F',
            'pergunta':['Geral','Futebol','Filmes e séries','Música','Brasil','Animais e natureza','Engraçada'],
        }
        category_filter=category_map.get(kind)
        category,question,answer,_pts=choose_question(gid, category=category_filter)
...
async def home(c):
    if not await require_callback(c):return
    await safe_edit(c.message, '🛠 <b>PAINEL ADMINISTRATIVO — PIT DIVERSÃO</b>\n\nEscolha uma opção:',reply_markup=home_kb()); await c.answer()
...
async def groups(c):
    ...
    await safe_edit(c.message, text,reply_markup=b.as_markup());await c.answer()

@router.callback_query(F.data=='adm:choose')
async def choose(c):
    if not await require_callback(c):return
    await safe_edit(c.message, '⚙️ <b>ESCOLHA O GRUPO</b>\n\nTudo será configurado individualmente:',reply_markup=group_kb());await c.answer()
...
async def group(c):
    ...
    if not g:return await c.answer('Grupo não encontrado.',show_alert=True)
    await safe_edit(c.message, show_group_text(g),reply_markup=settings_kb(gid));await c.answer()

async def refresh(c,gid,msg=''):
    g=get(gid); await safe_edit(c.message, show_group_text(g)+(f'\n\n✅ {msg}' if msg else ''),reply_markup=settings_kb(gid))
...
async def prizes(c):
    ...
    await safe_edit(c.message, text,reply_markup=group_kb());await c.answer()
...
async def status(c):
    ...
    b=InlineKeyboardBuilder();b.button(text='⬅️ Voltar',callback_data='adm:home');await safe_edit(c.message, text,reply_markup=b.as_markup());await c.answer()
