...
async def interact_send(c):
    ...
    if kind=='visual':
        ...
        if kind=='charada':
            ...
            pool=QUESTIONS
        category,question,answer,_pts=random.choice(pool)
        from handlers.challenge import send_text_challenge
        await send_text_challenge(c.bot,gid,category,question,answer,points,'INTERAÇÃO AGORA')
    await c.message.edit_text(
...
async def save_config(m:Message,state:FSMContext):
    ...
    try:
        if kind=='hours':
            ...
            if value is None:
                raise ValueError
            setting(gid,kind,value);msg='Horário salvo.'
        elif kind=='prize':
