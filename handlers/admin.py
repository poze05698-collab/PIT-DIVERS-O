...
async def safe_edit(c, text, reply_markup=None):
    """Edita a tela do painel sem quebrar quando ela já está igual."""
    if not c.message:
        return False
    try:
        await safe_edit(c, text, reply_markup=reply_markup)
        return True
...
async def home(c):
    if not await require_callback(c):return
    await safe_edit(c, '🛠 <b>PAINEL ADMINISTRATIVO — PIT DIVERSÃO</b>\n\nEscolha uma opção:',reply_markup=home_kb()); await c.answer()
...
async def group(c):
    ...
    if not g:return await c.answer('Grupo não encontrado.',show_alert=True)
    await safe_edit(c, show_group_text(g),reply_markup=settings_kb(gid));await c.answer()

async def refresh(c,gid,msg=''):
    g=get(gid); await safe_edit(c, show_group_text(g)+(f'\n\n✅ {msg}' if msg else ''),reply_markup=settings_kb(gid))
...
async def prizes(c):
    ...
    await safe_edit(c, text,reply_markup=group_kb());await c.answer()
...
async def status(c):
    ...
    b=InlineKeyboardBuilder();b.button(text='⬅️ Voltar',callback_data='adm:home');await safe_edit(c, text,reply_markup=b.as_markup());await c.answer()
