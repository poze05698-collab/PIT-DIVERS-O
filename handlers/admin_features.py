...
from database.features import settings, toggle, set_value, release_chest, chest_release_status, set_chest_message_id
...
def sys_text(gid):
    ...
    return (f'⚙️ <b>SISTEMAS — {g["title"] if g else "Grupo"}</b>\n\n'
            f'🟢 = ligado | 🔴 = desligado\n\n'
            f'⭐ XP: {"🟢" if s["xp_enabled"] else "🔴"}\n'
            f'🔥 Combos: {"🟢" if s["combo_enabled"] else "🔴"}\n'
            f'🏅 Conquistas: {"🟢" if s["achievements_enabled"] else "🔴"}\n'
            f'🎯 Missões: {"🟢" if s["missions_enabled"] else "🔴"}\n'
            f'🎁 Baú: {"🟢" if s["chest_enabled"] else "🔴"}\n'
            f'🧠 Memória: {"🟢" if s["memory_enabled"] else "🔴"}\n'
            f'🎉 Eventos: {"🟢" if s["events_enabled"] else "🔴"}\n'
            f'📊 Ranking categoria: {"🟢" if s["category_rank_enabled"] else "🔴"}\n'
            f'💬 Mensagens: {"🟢" if s["victory_messages_enabled"] else "🔴"}\n'
            f'📖 Como funciona: {"🟢" if s["howto_enabled"] else "🔴"}')
...
async def toggle_cb(c):
    ...
    # Ao ligar missões, avisa o grupo imediatamente.
...
async def release_chest(c, state: FSMContext):
    ...
    release, reason = release_chest(gid, int(s['chest_duration_minutes'] or 10))
    if not release:
        messages={
    ...
    try:
        ...
        set_chest_message_id(release['id'], sent.message_id)
    except Exception:
