...
async def toggle_cb(c):
    if not await cguard(c): return await c.answer('Sem permissão.',show_alert=True)
    _,_,gid,field=c.data.split(':'); toggle(int(gid),field); await c.message.edit_text(sys_text(int(gid)),reply_markup=sys_kb(int(gid))); await c.answer('Configuração salva.')

@router.callback_query(F.data.startswith('afs:p:'))
async def prizes(c):
    if not await cguard(c): return await c.answer('Sem permissão.',show_alert=True)
    gid=int(c.data.split(':')[2]); g=get(gid); b=InlineKeyboardBuilder()
    for i in range(1,4): b.button(text=f'🥇🥈🥉 Prêmio {i}',callback_data=f'afs:pp:{gid}:{i}')
...
async def prize_prompt(c,state: FSMContext):
    if not await cguard(c): return await c.answer('Sem permissão.',show_alert=True)
    _,_,gid,pos=c.data.split(':'); await state.set_state(FS.value); await state.update_data(kind='prize'+pos,gid=int(gid)); await c.message.answer(f'🎁 Envie o prêmio do <b>{pos}º lugar</b>.\nEx.: R$ 100 ou Banca 100'); await c.answer()

@router.callback_query(F.data.startswith('afs:v:'))
async def values(c):
    if not await cguard(c): return await c.answer('Sem permissão.',show_alert=True)
    gid=int(c.data.split(':')[2]); s=settings(gid); b=InlineKeyboardBuilder()
    for field,label in [('xp_per_answer','XP por acerto'),('combo_bonus','Bônus do combo'),('chest_points','Pontos do baú'),('mission_target','Meta da missão'),('battle_points','Pontos da batalha'),('event_multiplier','Multiplicador do evento')]: b.button(text=label,callback_data=f'afs:vv:{gid}:{field}')
