...
from database.features import settings, toggle, set_value
...
async def toggle_cb(c):
    if not await cguard(c): return await c.answer('Sem permissão.',show_alert=True)
    _,_,gid,field=c.data.split(':'); gid=int(gid); enabled_now=toggle(gid,field)
    if enabled_now and field in {'battles_enabled','missions_enabled'}:
        messages={'battles_enabled':'⚔️ <b>BATALHA 1x1 LIBERADA!</b>\n\nEntre na batalha e convide seu amigo para uma disputa! 🔥\n\nUse <b>/batalha</b> respondendo à mensagem do seu amigo.','missions_enabled':'🎯 <b>MISSÕES DIÁRIAS LIBERADAS!</b>\n\nParticipe dos desafios e complete sua missão do dia! 🔥\n\nUse <b>/missao</b> para acompanhar o progresso.'}
...
async def chest_prompt(c,state: FSMContext):
    if not await cguard(c): return await c.answer('Sem permissão.',show_alert=True)
    gid=int(c.data.split(':')[2]); s=settings(gid)
    await state.set_state(FS.value); await state.update_data(kind='chest_release',gid=gid)
    await c.message.answer(f'🎁 <b>LIBERAR BAÚ</b>\n\nInforme por quantos minutos ficará ativo.\nEx.: <b>{s["chest_duration_minutes"]}</b>')
    await c.answer()
...
async def values(c):
    ...
    for field,label in [('xp_per_answer','XP por acerto'),('combo_bonus','Bônus do combo'),('chest_points','Pontos do baú'),('mission_target','Meta da missão'),('battle_points','Pontos da batalha'),('event_multiplier','Multiplicador do evento'),('chest_duration_minutes','⏱️ Duração do baú (min)')]: b.button(text=label,callback_data=f'afs:vv:{gid}:{field}')
...
async def save_value(m,state: FSMContext):
    ...
    if kind=='chest_release':
        try: minutes=int(text)
        except: return await m.answer('❌ Envie apenas o número de minutos.')
        if minutes<1 or minutes>1440: return await m.answer('❌ Use de 1 a 1440 minutos.')
        from database.features import release_chest
        result=release_chest(gid,minutes)
        if not result: return await m.answer('❌ O baú está desativado neste grupo.')
        try:
            await m.bot.send_message(gid, f'🎁 <b>BAÚ DIÁRIO LIBERADO!</b>\n\nO baú está disponível por <b>{minutes} minutos</b>!\n👇 Use /bau para abrir.')
        except Exception:
            pass
    elif kind.startswith('prize'):
