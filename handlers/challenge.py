...
async def send_text_challenge(target, chat_id, category, question, answer, points, title="DESAFIO"):
    options=build_options(answer, category)
    ...
    await target.answer(text, reply_markup=keyboard(cid, options))
    return cid
...
async def adivinha(m):
    ...
    await send_text_challenge(m,m.chat.id,q[0],q[1],q[2],q[3],"ADIVINHA")
...
async def desafio(m):
    ...
    q=random.choice(QUESTIONS)
    await send_text_challenge(m,m.chat.id,q[0],q[1],q[2],q[3],"DESAFIO")
...
async def novo(m,bot):
    ...
    cancel(m.chat.id)
    await send_text_challenge(m,m.chat.id,'Personalizado',parts[0].strip(),parts[1].strip(),pts,"DESAFIO PERSONALIZADO")
...
async def answer_button(callback: CallbackQuery):
    ...
    options=build_options(c['answer'],c['category'])
