...
async def send_random_now(bot, chat_id):
    ...
    category, question, answer, _, options = choose_question(chat_id)
...
async def answer_button(callback: CallbackQuery):
    ...
    import json
    try: stored=json.loads(c['options_json'] or '[]')
    except Exception: stored=[]
    options=build_options(c['answer'], c['category'], question=c['question'], supplied=stored)
