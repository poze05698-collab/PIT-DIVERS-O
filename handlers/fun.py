...
async def mat(m):
    ...
    if not season_is_active(m.chat.id): return await m.answer('🔒 A temporada foi encerrada. Inicie uma nova temporada pelo painel Admin.')
    from database.challenges import active
    if active(m.chat.id): return await m.answer('⏳ Já existe um desafio ativo neste grupo.')
    from services.questions import choose_question
    from handlers.challenge import send_text_challenge
    q=await asyncio.to_thread(choose_question,m.chat.id,category='Matemática')
    await send_text_challenge(m.bot,m.chat.id,q[0],q[1],q[2],max(1,q[3]),'🧮 MATEMÁTICA',q[4])

@router.message(Command('embaralhada'))
async def emb(m):
    w,s=scramble(); await m.answer(f'🔤 <b>PALAVRA EMBARALHADA</b>\n\n<b>{s}</b>\n💡 Começa com {w[0].upper()}')

@router.message(Command('forca'))
async def forca(m):
    w=hangman(); await m.answer(f'🧩 <b>FORCA</b>\n\n<code>{" ".join("_" for _ in w)}</code>\n💡 Dica: palavra ligada ao PIT DIVERSÃO.\n🔎 {len(w)} letras')
...
async def vf(m):
    ...
    from services.questions import choose_question
    from handlers.challenge import send_text_challenge
    q=await asyncio.to_thread(choose_question,m.chat.id,category='V/F')
    await send_text_challenge(m.bot,m.chat.id,q[0],q[1],q[2],max(1,q[3]),'❓ VERDADEIRO OU FALSO',q[4])
...
async def quiz(m):
    ...
    await send_text_challenge(m.bot,m.chat.id,q[0],q[1],q[2],max(1,q[3]),'🧠 QUIZ',q[4])
