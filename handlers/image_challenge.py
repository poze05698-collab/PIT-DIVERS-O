...
from database.challenges import active, create, win, cancel_id
...
async def send_image_challenge(message_or_bot, chat_id: int, points: int = 10):
    if active(chat_id):
        return False
    ...
    try:
        ...
        # Se o Telegram recusar o envio, não deixar o registro ativo.
        cancel_id(cid)
        raise
...
async def answer_image(callback: CallbackQuery):
    ...
    add(chat_id, callback.from_user.id, challenge["points"],reason=f'Desafio {challenge["category"]}')
    ...
    name = (
        ...
        else escape(callback.from_user.first_name or "Membro")
    )
    ...
    await callback.message.answer(
        ...
        f"👑 {name}\n"
        f"⭐ <b>+10 pontos</b>\n"
        f"📊 Total acumulado na temporada: <b>{total} pontos</b>" + (f"\n🔥 Combo: <b>{fx['combo']}</b>" if fx["combo"] else "")
    )
