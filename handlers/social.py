...
async def perfil(m):
 upsert(m.from_user);f,fi=counts(m.from_user.id);u=get(m.from_user.id);n='@'+u['username'] if u['username'] else u['first_name'];await m.answer(f'👤 <b>PERFIL</b>\n\n{n}\n🆔 <code>{u["id"]}</code>\n❤️ Seguidores: {f}\n➡️ Seguindo: {fi}')
...
async def quem(m):
 ...
 u=m.reply_to_message.from_user;upsert(u);f,fi=counts(u.id);await m.answer(f'👤 <b>{u.first_name}</b>\n🆔 <code>{u.id}</code>\n❤️ {f} seguidores\n➡️ {fi} seguindo')
...
async def seguir(m):
 ...
 u=m.reply_to_message.from_user;await m.answer('❤️ Agora você segue '+u.first_name if follow(m.from_user.id,u.id) else 'ℹ️ Você já segue essa pessoa.')
...
async def parar(m):
 ...
 u=m.reply_to_message.from_user;await m.answer('💔 Você deixou de seguir '+u.first_name if unfollow(m.from_user.id,u.id) else 'ℹ️ Você não seguia essa pessoa.')
...
async def reagir(m):
 ...
 kind=(m.text or '').partition(' ')[2].strip() or '❤️';reaction(m.chat.id,m.from_user.id,kind);await m.answer(f'{kind} Reação registrada!')
