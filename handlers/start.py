...
async def como_funciona_natural(message: Message):
    if message.chat.type not in {"group", "supergroup"} or not message.text:
        return
    t=" ".join(message.text.lower().strip().split())
    triggers={"como funciona","como funciona?","como funciona o bot","como participar","como eu participo","o que esse bot faz","como ganhar pontos","quero participar"}
