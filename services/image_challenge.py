...
def make_sequence_image(answer: int, seed: int = 0, start: int | None = None, step: int | None = None):
    image = _base("DESAFIO VISUAL", "🔺 COMPLETE A SEQUÊNCIA")
    draw = ImageDraw.Draw(image, "RGBA")
    rng = random.Random(seed)
    start = int(start) if start is not None else rng.randint(2,8)
    step = int(step) if step is not None else rng.randint(2,9)
