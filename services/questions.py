...
def choose_question(group_id=None, category=None):
    ...
    # Open Trivia DB entra como fonte complementar. A biblioteca local continua
    # sendo a fonte principal; se a internet/API falhar, o bot segue normalmente.
    if random.randint(1, 100) <= 40:
        try:
            from services.opentdb import choose as choose_opentdb
            remote = choose_opentdb()
            if remote:
                return remote
        except Exception:
            pass
