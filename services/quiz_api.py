...
def normalize(item):
    ...
    # Nunca corte a lista de modo que o gabarito desapareça.
    if len(options)>6:
        wrong=[x for x in options if x != answer]
        options=[answer] + wrong[:5]
...
def _load():
    global _CACHE,_API_RETRY_AFTER,_USING_FALLBACK
    for url in API_URLS:
        try:
...
def fetch_question(category=None):
    global _RECENT_IDS
    # Enquanto a API estiver indisponível, use o fallback sem bloquear o bot.
    # Quando o intervalo de retry terminar, tente a API novamente mesmo que
    # `_CACHE` ainda contenha as perguntas de emergência.
    if not _CACHE:
        candidates=_load()
    ...
    if category:
        ...
        if filtered:
            ...
            # Se a API estiver funcionando mas não tiver a categoria pedida,
            # use o fallback específico em vez de entregar uma categoria aleatória.
            fallback=[q for q in _fallback_questions() if q["category"].lower() in wanted]
            if fallback:
                candidates=fallback
...
def refresh():
    global _CACHE,_API_RETRY_AFTER,_USING_FALLBACK
    _CACHE=[]; _API_RETRY_AFTER=0; _USING_FALLBACK=False
