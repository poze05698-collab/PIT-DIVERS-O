...
from database.features import close_season, settings, expired_chest_messages, mark_chest_message_deleted
    ...
    while True:
        try:
            ...
            async def process_group(g):
                ...
                if inside and bool(settings(gid)['events_enabled']) and random.randint(1,180)==1:
                    from database.connection import db
                    with db() as c:
            ...
            # Fechamento semanal independente do horário de interação do grupo.
            if now.weekday()==6 and now.hour==23 and now.minute==59:
                week=__import__('database.points',fromlist=['wk']).wk()
                for g in groups:
                    gid=g['id']
                    ...
                    try:
                        rows=close_season(gid,week,prizes)
                        if rows:
                        ...
                        log.exception('Falha no fechamento semanal do grupo %s',gid)
                    seen.add(marker)
