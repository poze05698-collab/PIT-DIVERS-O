...
from database.features import close_season, top3, settings, deactivate_expired_chests, expired_chest_messages
        ...
        try:
            ...
            async def process_group(g):
                ...
                try:
                    ...
                    from database.features import current_multiplier
                    for chest_gid, chest_message_id in expired_chest_messages():
                        try:
                            await bot.delete_message(chest_gid, chest_message_id)
                        except Exception:
                            log.warning('Não foi possível apagar postagem do baú %s no grupo %s', chest_message_id, chest_gid)
                    with db() as c:
